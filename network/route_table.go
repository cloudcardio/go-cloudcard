...
func (table *RouteTable) Start() {
	logging.CLog().Info("Starting cloudcardService RouteTable Sync...")
...
func (table *RouteTable) syncLoop() {
	...
	logging.CLog().Info("Started cloudcardService RouteTable Sync.")
	...
	for {
		select {
		case <-table.quitCh:
			logging.CLog().Info("Stopped cloudcardService RouteTable Sync.")
			return
...
func (table *RouteTable) Stop() {
	logging.CLog().Info("Stopping cloudcardService RouteTable Sync...")
