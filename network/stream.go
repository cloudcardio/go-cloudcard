...
import (
	netpb "cloudcard.pro/cloudcardio/go-cloudcard/network/pb"
...
const (
	ClientVersion  = "0.1.0"
	cloudcardProtocolID = "/cloudcard/1.0.0"
	HELLO          = "hello"
	OK             = "ok"
	BYE            = "bye"
	SYNCROUTE      = "syncroute"
...
type Stream struct {
	...
	highPriorityMessageChan   chan *cloudcardMessage
	normalPriorityMessageChan chan *cloudcardMessage
	lowPriorityMessageChan    chan *cloudcardMessage
...
func newStreamInstance(pid peer.ID, addr ma.Multiaddr, stream network.Stream, node *Node) *Stream {
	return &Stream{
		...
		highPriorityMessageChan:   make(chan *cloudcardMessage, 2*1024),
		normalPriorityMessageChan: make(chan *cloudcardMessage, 2*1024),
		lowPriorityMessageChan:    make(chan *cloudcardMessage, 2*1024),
...
func (s *Stream) handleMessage(message *cloudcardMessage) error {
...
func (s *Stream) onHello(message *cloudcardMessage) error {
...
func (s *Stream) onOk(message *cloudcardMessage) error {
...
func (s *Stream) onBye(message *cloudcardMessage) error {
	logging.VLog().WithFields(logrus.Fields{
		"stream": s.String(),
...
func (s *Stream) onSyncRoute(message *cloudcardMessage) error {
	return s.RouteTable()
}
...
func (s *Stream) onRouteTable(message *cloudcardMessage) error {
	data, err := s.getData(message)
	if err != nil {
		return err
	}
...
func (s *Stream) WriteMessage(messageName string, data []byte, reservedClientFlag byte) error {
	...
	message, err := NewcloudcardMessage(s.node.config.NetworkID, s.node.config.ChainID, reserved, CurrentVersion, messageName, data)
	if err != nil {
		return err
	}

	return s.WritecloudcardMessage(message)
}
...
func (s *Stream) SendMessage(messageName string, data []byte, priority int) error {
	message, err := NewcloudcardMessage(s.node.config.NetworkID, s.node.config.ChainID, s.reservedFlag, CurrentVersion, messageName, data)
	if err != nil {
		return err
	}
	...
	for {
		...
		for {
			if message == nil {
				...
				if len(messageBuffer) < cloudcardMessageHeaderLength {
					// continue reading.
					break
				}

				message, err = ParsecloudcardMessage(s.node.config.NetworkID, messageBuffer)
				if err != nil {
					s.Bye()
					return
				}
				...
				messageBuffer = messageBuffer[cloudcardMessageHeaderLength:]
			}
	...
	for {
		...
		case <-s.messageNotifChan:
			...
			case message := <-s.highPriorityMessageChan:
				s.WritecloudcardMessage(message)
				continue
			default:
			}
...
func (s *Stream) getData(message *cloudcardMessage) ([]byte, error) {
	var data []byte
