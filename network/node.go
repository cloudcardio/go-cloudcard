...
func (node *Node) startHost() error {
	...
	host.SetStreamHandler(cloudcardProtocolID, node.onStreamConnected)
	node.host = host
...
func GenUpgrader(n *swarm.Swarm) *tptu.Upgrader {
	...
	stMuxer.AddTransport(cloudcardProtocolID, yamux.DefaultTransport)
...
func (node *Node) Start() error {
	logging.CLog().Info("Starting cloudcardService Node...")
	...
	}).Info("Started cloudcardService Node.")
...
func (node *Node) Stop() {
	logging.CLog().WithFields(logrus.Fields{
		"id":                node.ID(),
		"listening address": node.host.Addrs(),
	}).Info("Stopping cloudcardService Node...")
